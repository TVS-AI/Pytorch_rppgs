...
class PhysNet_2D_CNN_LSTM(nn.Module):
    def __init__(self):
        super(PhysNet_2D_CNN_LSTM, self).__init__()
        ...
        self.ConvBlock10 = nn.Conv2d(64, 1, 1, stride=1, padding=0)

        self.MaxpoolSpa = nn.MaxPool2d((2, 2), stride=2)

        self.poolspa = nn.AdaptiveAvgPool2d(1)

    def forward(self, x):  # x [3, 128,128]
        x_visual = x
        [batch, channel, width, height] = x.shape
        # x [3, 128,128]
        x = self.ConvBlock1(x)  # x [16, 128,128]

        x = self.MaxpoolSpa(x)  # x [32, 64, 64]
        x = self.ConvBlock2(x)  # x [64, 64, 64]
        x = self.ConvBlock3(x)  # x [64, 64, 64]

        x = self.MaxpoolSpa(x)  # x [64, 32,32]
        x = self.ConvBlock4(x)  # x [64, 32, 32]
        x = self.ConvBlock5(x)  # x [64, 32, 32]

        x = self.MaxpoolSpa(x)  # x [64, 16, 16]
        x = self.ConvBlock6(x)  # x [64, 16, 16]
        x = self.ConvBlock7(x)  # x [64, 16, 16]

        x = self.MaxpoolSpa(x)  # x [64, 8, 8]
        x = self.ConvBlock8(x)  # x [64, 8, 8]
        x = self.ConvBlock9(x)  # x [64, 8, 8]

        x = self.poolspa(x)  # x [64, 1,1]
        LSTM_x = x.view(x.shape[0], 1, -1)
        x = self.LSTM(LSTM_x)
        x = self.ConvBlock10(x)  # x [1, 1, 1]
