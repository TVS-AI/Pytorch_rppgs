...
def test(model, test_loader, check_model, device):
    ...
    checkpoint = torch.load(check_model + "/checkpoint_17d_17h_58m.pth")
    ...
    for i in range(0, 1):
        with torch.no_grad():
            ...
            for k, (avg, mot, lab) in tqdm(enumerate(test_loader), desc="test ", total=len(test_loader)):
            ...
            # Save Attention Mask
            data_file = h5py.File('/home/js/Desktop/Data/Pytorch_rppgs_save/preprocessing/mask/COHFACE_mask_train.hdf5', 'w')
    ...
    # corr = np.corrcoef(pulse_pred, target)
    # print(corr)
    # k = 0
    # # matplot---------------------------------------------------------------------
    # plt.rcParams["figure.figsize"] = (14, 5)
    # plt.plot(range(len(pulse_pred[k:k + 300])), pulse_pred[k:k + 300], label='inference')
